...
import {ReentrancyGuard} from "@openzeppelin/contracts/security/ReentrancyGuard.sol"; //When working with external contracts best practice to use the nonReentrant modifier within this contract
...
contract DSCEngine is ReentrancyGuard{
    ...
    event CollateralDeposited(addressindexed user,address indexed token,uint256 indexed amount);
    ...
    constructors(address[] memory tokenAddresses, address[] priceFeedAddresses, address dscAddress) {
    ...
    ) external moreThanZero(amountCollateral) isAllowToken(tokenCollateralAddress) nonReentrant {

        s_collateralDeposited[msg.sender][tokenCollateralAddress] += amountCollateral;
        emit CollateralDeposited(msg.sender,tokenCollateralAddress,amountCollateral);
        bool success = IERC20(tokenCollateralAddress).transferFrom(msg.sender),address(this),amountCollateral);
        if(!success){  //Transferring from user(ie: msg.sender) to this contract (ie:address(this)) with the amount of ammountCollateral
